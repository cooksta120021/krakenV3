...
_ADAPTER = KrakenAdapter("", "", rate_limiter=GLOBAL_RATE_LIMITER)


def fetch_assets():
    data = _ADAPTER.public_request("Assets", weight=1.0)
    return data.get("result") or {}


def fetch_asset_pairs():
    data = _ADAPTER.public_request("AssetPairs", weight=1.0)
    return data.get("result") or {}
